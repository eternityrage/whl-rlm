...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "🐋 Witness the Majesty of Whales\n\n#whales #ocean #nature #wildlife"
        
        # Ensure message is not empty (VK requirement)
        if not message.strip():
            message = "🐋 New Majestic Whale Video!"
        ...
        video = upload.video(
            ...
            name=title or 'Majestic Whale Video',
...
def main():
    ...
    description = sys.argv[2] if len(sys.argv) > 2 else "Witness the Majesty of Whales 🐋🌊"
    title = sys.argv[3] if len(sys.argv) > 3 else "Majestic Whales"
